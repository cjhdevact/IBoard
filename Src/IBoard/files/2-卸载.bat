...
rd /s /q "%systemdrive%\ProgramData\Microsoft\Windows\Start Menu\Programs\图片展示小工具"
...
if exist "%userprofile%\..\Public\Desktop\图片展示小工具.lnk" del /q "%userprofile%\..\Public\Desktop\图片展示小工具.lnk"

choice /C YN /T 5 /D N /M "是(Y)否(N)删除自定义设置（5秒后自动选择N）"
if errorlevel 1 set a=1
if errorlevel 2 set a=2
if "%a%" == "1" Reg delete HKCU\Software\CJH\IBoard /f

Reg delete HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\IBoard /v DisplayIcon /f
Reg delete HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\IBoard /v DisplayName /f
